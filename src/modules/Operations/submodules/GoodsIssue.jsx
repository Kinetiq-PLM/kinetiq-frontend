                ...
                {documentItems.map((item, index) => (
                    <tr key={item.content_id || index}>
                          ...
                          <span readOnly style={{ cursor: 'not-allowed' }}>
                            {item.item_id && duplicateDetails[item.item_id]?.[0]
                              ? `${duplicateDetails[item.item_id][0].price.toFixed(2)}${
                                  duplicateDetails[item.item_id][0].date
                                    ? ` (Purchased: ${duplicateDetails[item.item_id][0].date})`
                                    : ''
                                }`
                              : '0.00'}
                          </span>
                      ...
                      <td className="item-date-input">
                        {(() => {
                          const matchedOption = itemOptions.find(opt => opt.name === item.item_name);
                          const itemType = item.item_type || matchedOption?.type;
