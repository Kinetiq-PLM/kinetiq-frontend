...
const GoodsIssue = ({ onBack, onSuccess, selectedData, selectedButton, employee_id }) => {
  ...
  const fetchVendors = async () => {
    try {
      setLoading(true);
      const response = await fetch("https://js6s4geoo2.execute-api.ap-southeast-1.amazonaws.com/dev/supplier/");
  ...
  useEffect(() => {
    const fetchNextDocumentIds = async () => {
      if (isCreateMode) {
        try {
          const response = await fetch('https://js6s4geoo2.execute-api.ap-southeast-1.amazonaws.com/dev/goods-tracking/get-next-doc-ids/');
  ...
  const handleInputChange = async (e, index, field) => {
    ...
    if (index !== updatedItems.length - 1 && currentItem.item_name.trim() === '') {
      ...
      try {
        await fetch(`https://js6s4geoo2.execute-api.ap-southeast-1.amazonaws.com/dev/document-item/${currentItem.content_id}/`, {
          method: 'PATCH',
          headers: {
  ...
  const reloadDocumentItems = async () => {
    try {
      const response = await fetch(`https://js6s4geoo2.execute-api.ap-southeast-1.amazonaws.com/dev/goods-tracking/${selectedData.document_id}/`);
    ...
    if (isRowFilled(lastRow)) {
      try {
        ...
        if (lastRow.item_id.startsWith("ADMIN-MATERIAL")) {
          ...
          const productDocuResponse = await fetch('https://js6s4geoo2.execute-api.ap-southeast-1.amazonaws.com/dev/create-items/create-product-docu-item/', {
            method: 'POST',
        ...
        const createResponse = await fetch('https://js6s4geoo2.execute-api.ap-southeast-1.amazonaws.com/dev/create-items/create-document-item/', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  useEffect(() => {
    fetch('https://js6s4geoo2.execute-api.ap-southeast-1.amazonaws.com/dev/item-data/')
      .then(res => res.json())
      .then(data => {
        const options = [];
  ...
  const handleCreateDocument = async () => {
    try {
      ...
      // Call the create API
      const response = await fetch('https://js6s4geoo2.execute-api.ap-southeast-1.amazonaws.com/dev/goods-tracking/custom-create/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
                      ...
                      <td>
                        {!item.item_id?.startsWith('ADMIN-ASSET') ? (
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              handleInputChange(e, index, 'purchase_date');
                            }}
                            readOnly
                            style={{ cursor: 'not-allowed' }}
                          />
