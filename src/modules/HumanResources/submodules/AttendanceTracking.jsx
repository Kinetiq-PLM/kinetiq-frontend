...
const AttendanceTracking = () => {
  ...
  const fetchAttendance = async () => {
    setLoading(true);
    try {
      const res = await axios.get("http://127.0.0.1:8001/api/attendance_tracking/attendance_tracking/");
  ...
  const fetchCalendarDates = async () => {
    setLoading(true);
    try {
      const res = await axios.get("http://127.0.0.1:8001/api/calendar_dates/calendar_dates/");
      // Ensure we're setting an array, even if the response is unexpected
      const responseData = Array.isArray(res.data) ? res.data : [];
...
const fetchOvertimeRequests = async () => {
  ...
  try {
    ...
    const res = await axios.get("http://127.0.0.1:8001/api/overtime_requests/");
    const responseData = Array.isArray(res.data) ? res.data : [];
