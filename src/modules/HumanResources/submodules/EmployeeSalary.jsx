...
const EmployeeSalary = () => {
  ...
  const fetchSalaryData = async () => {
    setLoading(true);
    try {
      const res = await axios.get("http://127.0.0.1:8001/api/employee_salary/employee_salary/");
  ...
  const handleEditSalary = async (salary) => {
    ...
    if (!employeeType) {
      try {
        const empRes = await axios.get(`http://127.0.0.1:8001/api/employees/${salary.employee_id}/`);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      await axios.patch(
        `http://127.0.0.1:8001/api/employee_salary/employee_salary/${editingSalary.salary_id}/`,
        payload
      );
