...
const EmployeeSalary = () => {
  ...
  const fetchSalaryData = async () => {
    ...
    try {
      ...
      for (const employeeId of employeeIds) {
        try {
          const empRes = await axios.get(`http://127.0.0.1:8000/api/employees/employee/${employeeId}/`);
  ...
  const handleEditSalary = async (salary) => {
    ...
    if (!employeeType) {
      try {
        const empRes = await axios.get(`http://127.0.0.1:8000/api/employees/employee/${salary.employee_id}/`);
