...
const EditPickingModal = ({ show, onClose, pickingList, onSave, employees, warehouses, onStatusUpdate }) => {
  ...
  const fetchDeliveryNotesInfo = async (orderID) => {
    ...
    try {
      const response = await fetch(`http://r7d8au0l77.execute-api.ap-southeast-1.amazonaws.com/dev/api/delivery-notes/order/${orderID}/`);
  ...
  const fetchPickingItems = async () => {
    ...
    try {
      ...
      if (!response.ok || data.length === 0) {
        const createResponse = await fetch(`http://r7d8au0l77.execute-api.ap-southeast-1.amazonaws.com/dev/api/picking-lists/${pickingList.picking_list_id}/create-items/`, {
          method: 'POST'
        });
        ...
        response = await fetch(`http://r7d8au0l77.execute-api.ap-southeast-1.amazonaws.com/dev/api/picking-lists/${pickingList.picking_list_id}/items/`);
  ...
  const toggleItemPicked = async (item) => {
    ...
    try {
      ...
      const response = await fetch(`http://r7d8au0l77.execute-api.ap-southeast-1.amazonaws.com/dev/api/picking-items/${item.picking_item_id}/update/`, {
