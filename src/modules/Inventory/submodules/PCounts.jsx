        ...
        fetch("http://127.0.0.1:8000/api/cyclic_counts/")
            ...
            .then((data) => {
                ...
                data.forEach((item, index) => {
                    console.log(`Item ${index} - Product Name: ${item.product_name}, Debug Info:`, item.debug_info);
                });
                setPcounts(data);
                setLoading(false);
            })
                            ...
                            <table className="w-full table-layout:fixed text-center cursor-pointer">
                                <thead>
                                    <tr className="border-b border-gray-300">
                                        {['Product Name', 'P-Count', 'Date Checked', 'Employee', 'Status'].map((header) => (
                        ...
                        <div className="grid grid-rows-4 gap-3 justify-between h-full">
                            ...
                            >
                                ...
                                {["Completed", "In Progress", "Open", "Closed"].map((s) => (
                                    <option key={s} value={s}>{getStatusDisplayValue(s)}</option>
                                ))}
                            </select>
