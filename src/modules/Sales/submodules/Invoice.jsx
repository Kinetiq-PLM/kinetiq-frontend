...
import DeliveryListModal from "./../components/Modals/Lists/DeliveryList";
...
const Invoice = ({ loadSubModule, setActiveSubModule }) => {
  const { showAlert } = useAlert();

  const copyFromOptions = ["Delivery"];
  const copyToOptions = ["Return"];
  ...
  const columns = [
    ...
    { key: "markup_price", label: "Price", editable: false },
  ...
  useEffect(() => {
    ...
    const modalActions = {
      Delivery: setIsDeliveryListOpen,
    };
  ...
  useEffect(() => {
    if (copyFromModal === "Delivery" && selectedDelivery) {
      setOrderInfo(selectedDelivery);
      setCopyFromModal("");
